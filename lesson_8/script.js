...
const game = {
    ...
    pause() {
        ...
        if (element.classList.contains(GAME_STATUS_STARTED)) {
            this.setGameStatus(GAME_STATUS_PAUSED);
        } else if (element.classList.contains(GAME_STATUS_PAUSED)) {
            this.setGameStatus(GAME_STATUS_STARTED);
        }
    },
...
const snake = {
    ...
    /**
     * Функция определяет столкновение змейки самой с собой.
     */
    crash() {
        ...
        for (i = 0; i < this.parts.length - 1; i++) {
            ...
            if (head.top == part.top && head.left == part.left) {
                return true;
            }
        }
        return false;
...
function init() {
    ...
    setInterval(game.autoStep, config.speed);
